...
function deleteData(req, res) {
    ...
    tp.sql(`EXEC Firebase..sp_ProductRankSyncData 'delete'`).execute().then(function (results) {
        let all = results.length;
        if (all > 0) {
            let success = 0;
            let arr = [];
            results.forEach(result => {
                db.ref(`product/TH/0001/summary/sale/rank/${result.id}`).remove(function (error) {
                    success++;
                    if (error) {
                    ...
                    if (success >= all) {
                        ...
                        tp.sql(`DELETE FROM Firebase..ProductRank WHERE product IN (${arr.toString()})`).execute();
...
function updateData(req, res, msg) {
    let tp = require('tedious-promises');
    tp.setConnectionConfig(require(`${__dirname}/../../.config/www/mssql.json`));
    tp.sql(`EXEC Firebase..sp_ProductRankSyncData 'update'`).execute().then(function (results) {
        let all = results.length;
        if (all > 0) {
            let success = 0;
            let arr = [];
            results.forEach(result => {
                ...
                db.ref(`product/TH/0001/summary/sale/rank/${result.id}`).update(json, function (error) {
                    success++;
                    if (error) {
                    ...
                    if(success >= all) {
                        ...
                        res.send(msg);
                        tp.sql(`UPDATE Firebase..ProductRank SET isSync = 1, syncDate = GETDATE() WHERE product IN (${arr.toString()})`).execute();
