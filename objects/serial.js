...
function deleteData(req, res) {
    ...
    tp.sql(`EXEC Firebase..sp_SerialSyncData 'delete'`).execute().then(function (results) {
        let all = results.length;
        if (all > 0) {
            let success = 0;
            let arr = [];
            results.forEach(result => {
                db.ref(`serial/TH/0001/stock/${result.id}`).remove(function (error) {
                    success++;
                    if (error) {
...
function updateData(req, res, msg) {
    let tp = require('tedious-promises');
    tp.setConnectionConfig(require(`${__dirname}/../../.config/www/mssql.json`));
    tp.sql(`EXEC Firebase..sp_SerialSyncData 'update'`).execute().then(function (results) {
        let all = results.length;
        if (all > 0) {
            let success = 0;
            let arr = [];
            results.forEach(result => {
                db.ref(`serial/TH/0001/info/${result.id}`).update({
                    billNo: ''+result.buyNo,
                    keyBillProduct: ''+result.buyNo+':'+result.product,
                    poDate: ''+result.receivedDate,
                    product: ''+result.product,
                });

                db.ref(`serial/TH/0001/stock/${result.id}`).update({
                    product: result.product
                }, function (error) {
                    success++;
                    if (error) {
